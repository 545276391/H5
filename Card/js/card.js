let totalWidth = (document.documentElement.scrollWidth>document.documentElement.clientWidth) 
  ...
  const MONTHLY = 1;
	...
	function Card(base_cost,flow_province,flow_country,call_count,flow_province_unit_price,flow_country_unit_price,call_unit_price,card_type,daily_flow){
      ...
      this.daily_flow = daily_flow;
	    this.getExtraFlow = function(){
	      return this.daily_flow;
	    }
	}
 ...
 let tencent_big_king_card = new Card(19,0,0,0,1,1,0.1,DAILY,1);
  
 let tencent_heaven_king_card = new Card(59,0,0,800,1,1,0.1,DAILY,1);
 let alibaba_small_fish_card = new Card(9,0,0,0,1,2,0.1,DAILY,0.8);
 let alibaba_big_fish_card = new Card(19,0,1,100,1,2,0.1,DAILY,0.8);
 ...
 let jd_experience_card = new Card(5,0,0,0,1,2,0.1,DAILY,0.8);
 ...
 let baidu_small_sage_card = new Card(9,0,0,0,1,15,0.1,DAILY,0.8);
 let baidu_big_sage_card = new Card(19,0,1,100,1,10,0.1,DAILY,0.8);
 let baidu_beyond_sage_card = new Card(59,0,4,300,1,10,0.1,DAILY,0.8);
 ...
 let mi_1_189_daily_card = new Card(5,0,0,0,1,2,0.1,DAILY,0.8);
 let mi_3_189_daily_card = new Card(5,0,0,0,3,2,0.1,DAILY,10);
 let mi_1_10010_daily_card = new Card(5,0,0,0,1,1,0.1,DAILY,0.8);
 let mi_3_10010_daily_card = new Card(5,0,0,0,3,3,0.1,DAILY,10);
 ...
 let bilibili_22_card = new Card(22,0,1,100,1,1,0.1,DAILY,0.5);
 let bilibili_33_card = new Card(33,0,2,100,1,1,0.1,DAILY,0.5);
 let bilibili_small_tv_card = new Card(66,0,4,300,1,2,0.1,DAILY,0.5);
 let inke_small_in_card = new Card(29,0,1,0,1,2,0.1,DAILY,0.5);
 let inke_big_in_card = new Card(59,0,3,200,1,2,0.1,DAILY,0.5);
 let huajiao_small_huajiao_card = new Card(29,0,2,0,1,1,0.1,DAILY,0.5);
 let huajiao_big_huajiao_card = new Card(59,0,4,200,1,1,0.1,DAILY,0.5);
 
 let huajiao_pollen_card = new Card(10,0,0,0,2,100,0.1,DAILY,2);
 let youkutudou_small_cool_card = new Card(9,0,0,0,1,3,0.1,DAILY,0.5);
 let youkutudou_big_cool_card = new Card(29,0,2,0,1,2,0.1,DAILY,0.5);
 let mgtv_small_mang_card = new Card(9,0,0,0,1,10,0.1,DAILY,0.5);
 let mgtv_big_mang_card = new Card(29,0,2,0,1,10,0.1,DAILY,0.5);
 ...
 let toutiao_know_me_mini_card = new Card(9,0,0,0,1,2,0.1,DAILY,0.8);
 ...
 let tv189_kankan_card = new Card(6,0,0,0,1,100,0.1,DAILY,0.5);
 ...
 let woniu_small_card = new Card(19,0,1.5,0,1,12,0.1,DAILY,0.5);
 let woniu_big_card = new Card(59,0,4,200,1,10,0.1,DAILY,0.5);
 ...
 let media_information_app_card = new Card(9,0,0,0,1,100,0.1,DAILY,0.5);
 ...
 let wy163_small_platinum_card = new Card(9,0,0,0,1,2,0.1,DAILY,1);
 let wy163_big_platinum_card = new Card(19,0,1,100,1,2,0.1,DAILY,1);
 let wy163_beyond_platinum_card = new Card(69,0,5,300,1,2,0.1,DAILY,1);
 
 let wy163_red_9_card = new Card(9,0,0,0,1,2,0.1,DAILY,0.8);
 let wy163_red_19_card = new Card(19,0,1,100,1,2,0.1,DAILY,0.8);
 let lvmama_lvfen_card = new Card(9,0,1,0,1,2,0.1,DAILY,0.5);
 let other_card = new Card(9,0,0,0,1,2,0.1,DAILY,0.5);
 ...
 function showCardName(index){
     switch(index){
       ...
       case 88:
         return wy163_small_platinum_card;
       case 89:
         return wy163_big_platinum_card;
       case 90:
         return wy163_beyond_platinum_card;
       case 91:
         return wy163_red_9_card;
       case 92:
         return wy163_red_19_card;
       case 93:
         return lvmama_lvfen_card;
       case 94:
         return other_card;
       default:
         return "?";
  ...
  function CalculationFlowCost(card){
    ...
    //套餐内流量不够用开始计费
        ...
        if(card.card_type == DAILY){
            //套餐内无流量
            if(card.flow_country == 0 && card.flow_province == 0){
                //每天触发一次日租宝
                if(forecast_flow_count <= DAILY * card.daily_flow){
                    return DAILY * card.flow_province_unit_price;
                //每天触发多次日租宝
                }else{
                    return (DAILY + ((forecast_flow_count - DAILY * card.daily_flow)/card.daily_flow) ) * card.flow_province_unit_price;
                }
            //套餐内有流量  
            }else{
                //每天触发一次日租宝
                if(forecast_flow_count <= ((DAILY - (card.flow_country + card.flow_province) * (Math.floor(DAILY/forecast_flow_count))) * card.daily_flow)){
                    return (DAILY- (card.flow_country + card.flow_province)*(Math.floor(DAILY/forecast_flow_count))) * card.flow_province_unit_price;
                //每天触发多次日租宝
                }else{
                    return (DAILY - (card.flow_country + card.flow_province) * (Math.floor(DAILY/forecast_flow_count))) * card.flow_province_unit_price 
                    + ((forecast_flow_count - ((DAILY-(card.flow_country + card.flow_province) * (Math.floor(DAILY/forecast_flow_count))) * card.daily_flow))/card.daily_flow) * card.flow_province_unit_price;
                }
            }
        //月租卡
        }else{
     ...
     {name: 'tip', mapping : 'tip'},
      ...
      Ext.onReady(function(){
        ...
        let myData = [
           ...
           { cardGroup : "网易<br/>+<br/>联通", cardName : "小白金卡<br/>月租9元;<br/>省内流量1元/G,省外2元/G(升级后),0.1元/分钟", sumCost :CalculationSumCost(showCardName(88)), tip : "网易应用冕旒;<br/>半年后或预存200元升级;<br/>首月免月租,首充50元送50元及价值46元的网易应用权益包,17年12月至18年2月期间每月送1G国内流量"},
           { cardGroup : "网易<br/>+<br/>联通", cardName : "大白金卡<br/>月租19元,含100分钟通话、1G国内流量;<br/>省内流量1元/G,省外2元/G(升级后),0.1元/分钟", sumCost :CalculationSumCost(showCardName(89)), tip : "网易应用冕旒;<br/>半年后或预存200元升级;<br/>首月免月租,首充50元送50元及价值46元的网易应用权益包,17年12月至18年2月期间每月送1G国内流量"},
           { cardGroup : "网易<br/>+<br/>联通", cardName : "超白金卡<br/>月租69元,含300分钟通话、5G国内流量;<br/>省内流量1元/G,省外2元/G(升级后),0.1元/分钟", sumCost :CalculationSumCost(showCardName(90)), tip : "网易应用冕旒;<br/>半年后或预存200元升级;<br/>首月免月租,首充50元送50元及价值46元的网易应用权益包,17年12月至18年2月期间每月送1G国内流量"},
           
           { cardGroup : "网易<br/>+<br/>电信", cardName : "9元红卡<br/>月租9元;<br/>省内流量1元/800M,省外2元/800M,0.1元/分钟", sumCost :CalculationSumCost(showCardName(91)), tip : "网易应用冕旒;<br/>首月免月租,首充50元送50元"},
           { cardGroup : "网易<br/>+<br/>电信", cardName : "19元红卡<br/>月租19元,含100分钟通话、1G国内流量;<br/>省内流量1元/800M,省外2元/800M,0.1元/分钟", sumCost :CalculationSumCost(showCardName(92)), tip : "网易应用冕旒;<br/>首月免月租,首充50元送50元"},
           { cardGroup : "驴妈妈<br/>+<br/>电信", cardName : "驴粉卡<br/>月租9元,含1G国内流量;<br/>省内流量1元/500M,省外2元/500M,0.1元/分钟", sumCost :CalculationSumCost(showCardName(93)), tip : "赠送旅游优惠券;<br/>每充值100送25元国漫专用金;<br/>首月免月租,送40元话费分8月返还,首充50元送50元,另送1G国内流量"},
           { cardGroup : "未完待续...", cardName : "未完待续...(天国的小王卡)", sumCost :CalculationSumCost(showCardName(94)), tip : "未完待续..."}
        ];
